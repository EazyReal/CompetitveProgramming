...
signed main()
{
  ...
  //l
  priority_queue<Library, vector<Library>, ccmp> pq;
  rep(i, 0, L)
  {
    pq.push(l[i]);
  }
  ...
  int curt = 0;
  int curl = 0;
  while(curt < D && !pq.empty())
  {
    Library lib = pq.top(); pq.pop();
    lused[lib.id] = 1;
    ans[curl].X = lib.id;
    curt += lib.t;
    if(curt > D) break;
    ...
    rep(i, 0, L) if(!lused[i])
    {
      l[i].pri = 0;
      for(int j = 0; j < l[i].n; ++j)if(!used[l[i].bl[j]])
      {
        l[i].pri += b[l[i].bl[j]].pri;
      }
      pq.push(l[i]);
    }
  }
