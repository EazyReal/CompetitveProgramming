...
struct Library
{
  ...
  int id;
  vector<Book> bl;
  double pri; //sum of bl's pri, m, t
  //c1 * sigma pri * m + c3 * t
};
...
bool cmp_b(Book &a, Book &b)
{
  if (used[a.id]) return 0;
...
signed main()
{
  ...
  rep(j, 0, L)
  {
    ...
    for(int i = 0; i < li.n; ++i)
    {
      li.pri = max(li.pri, 1./(b[li.bl[i].id].freq+1.));
    }
  }
  //sort(all(l), cmp); //some sorting according to pri Library
