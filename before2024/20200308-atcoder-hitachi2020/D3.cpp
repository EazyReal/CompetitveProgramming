...
{
	if((a[x].X) * (a[y].Y+1) < (a[y].X) * (a[x].Y+1) ) return 1; //deadline
	else if((a[x].X) * (a[y].Y+1) == (a[y].X) * (a[x].Y+1) ) return a[x].X < a[y].X;
	return 0;
	//if(t[x] > t[y]) return 1;
...
inline ll m(int j) {return 1/(aa[i]+1);}
inline ll b(int j) {return -(aa[i]+bb[i]+1)/(aa[i]+1);}
