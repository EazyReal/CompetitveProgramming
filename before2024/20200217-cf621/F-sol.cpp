//https://codeforces.com/contest/1307/submission/71331133
#include<bits/stdc++.h>
using namespace std;
...
int main() {
	...
	auto query = [&](int a, int b) -> bool {
		...
		return restDist[pa].first + md <= K && restDist[pb].first + md <= K && dsu::getPar(restDist[pa].second) == dsu::getPar(restDist[pb].second);
	};
