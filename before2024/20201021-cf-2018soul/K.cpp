...
inline int tob(char c)
{
    return (c=='B');
}

inline int pid(int a, int b)
{
    return a*2+b;
}
...
void solve()
{
    ...
    cin >> n >> m; //n = k in problem,
    array<int, 3> a;
    array<char, 3> c;
    array<int, 3> b;
    rep(i, 0, m)
    {
        rep(i, 0, 3) //read input
        {
            cin >> a[i] >> c[i];
            --a[i];
            b[i] = tob(c[i]);
        }
        rep(i, 0, 3) //add must do edges
        {
            SAT.imply(pid(a[i], b[i]^1), pid(a[(i+1)%3], b[(i+1)%3]));
            SAT.imply(pid(a[i], b[i]^1), pid(a[(i+2)%3], b[(i+2)%3]));
        }
    }
    SAT.scc(2*n);
    bool ok=1;
