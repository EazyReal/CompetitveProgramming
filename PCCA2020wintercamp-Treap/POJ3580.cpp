...
//21277145	maxwill	3580	Accepted	6148K	1579MS	G++	5478B	2020-01-29 13:26:37
//https://amoshyc.github.io/ojsolution-build/poj/p3580.html style
...
void push(Node *a)
{
  if(a->rev)
  {
    if(a->lc) { swap(a->lc->lc, a->lc->rc); a->lc->rev^= 1;}
    if(a->rc) { swap(a->rc->lc, a->rc->rc); a->rc->rev^= 1;}
    a->rev = 0;
  }
  if(a->addv != 0)
  {
    if(a->lc) {a->lc->addv += a->addv; a->lc->val += a->addv; a->lc->minv += a->addv;}
    if(a->rc) {a->rc->addv += a->addv; a->rc->val += a->addv; a->rc->minv += a->addv;}
    a->addv = 0;
  }
  return;
}
...
void pull(Node *a)
{
  ...
  a->minv = a->val;
  if(a->lc) a->minv = min(a->minv, a->lc->minv);
  if(a->rc) a->minv = min(a->minv, a->rc->minv);
}
